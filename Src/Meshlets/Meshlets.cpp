// Meshlets.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
public:
    ...
    // CPU meshoptimizer meshlet. Kept only while building the GPU buffers.
    std::vector<meshopt_Meshlet> meshlets;

    // GPU-friendly meshlet. This matches the HLSL Meshlet struct exactly.
    struct GpuMeshlet
    {
    ...
    std::vector<unsigned char> meshletTriangleBytes;
...
public:
    ...
    void CreatePipeline()
    {
        auto meshShaderBlob = shaderCompiler.Compile(L"../../Assets/Shaders/Meshlets/Mesh.hlsl", L"MS", L"ms_6_5");
        auto pixelShaderBlob = shaderCompiler.Compile(L"../../Assets/Shaders/Meshlets/PixelShader.hlsl", L"PS", L"ps_6_0");
                ...
                vertices.push_back(
                    {
                        ...
                        { std::abs(x), std::abs(y), std::abs(z), 1.0f }
                    });
            }
        }
    ...
    void CreateIndexBuffer()
    {
        ...
        // We also create a GPU meshlet table whose triangle_offset points into this uint stream.
        gpuMeshlets.clear();
        gpuMeshlets.reserve(meshletCount);

        meshletTriangles.clear();
        meshletTriangles.reserve(indexCount / 3);
        ...
        for (uint32_t i = 0; i < meshletCount; ++i)
        {
            ...
            dst.triangle_offset = static_cast<uint32_t>(meshletTriangles.size());
            dst.vertex_count = src.vertex_count;
            dst.triangle_count = src.triangle_count;

            for (uint32_t t = 0; t < src.triangle_count; ++t)
            {
                const uint32_t byteOffset = src.triangle_offset + t * 3;

                const uint32_t a = meshletTriangleBytes[byteOffset + 0];
                const uint32_t b = meshletTriangleBytes[byteOffset + 1];
                const uint32_t c = meshletTriangleBytes[byteOffset + 2];

                const uint32_t packedTriangle = a | (b << 8) | (c << 16);
                meshletTriangles.push_back(packedTriangle);
            }

            gpuMeshlets.push_back(dst);
        }
    ...
    void OnRender()
    {
        ...
        auto barrierToRenderTarget = CD3DX12_RESOURCE_BARRIER::Transition(renderTargets[backBufferIndex], D3D12_RESOURCE_STATE_PRESENT, D3D12_RESOURCE_STATE_RENDER_TARGET);
        ...
        //  second cube
        commandList->SetPipelineState(pipelineState);
        commandList->SetGraphicsRootConstantBufferView(4, constBuffer2.m_buffer->GetGPUVirtualAddress());
        commandList->DispatchMesh(meshletCount, 1, 1);
    ...
    void Cleanup()
    {
        ...
        if (meshletVerticesBuffer)
            meshletVerticesBuffer->Release();
...
int main()
{
    WindowApp win = { 1280u, 720u, L"DX12 Meshlets" };
